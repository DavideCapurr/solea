...
Solea ne transmet aucune donnée personnelle à ses propres serveurs ou à des
tiers.
...
- **Location** : utilisée dans l'app uniquement pour appeler WeatherKit d'Apple
  (indice UV et météo). Apple gère la requête comme service système ; Solea
  n'enregistre ni ne transmet la position ailleurs.
  → *Non collectée par Solea.*
- **Health & Fitness** : Solea **écrit** Time in Daylight dans le HealthKit
...
- **Photos** : le journal photo enregistre les images choisies par l'utilisateur
  dans le SwiftData local (sandbox de l'app). Aucun upload vers Solea ou tiers.
  → *Non collectée par Solea.*
- **Game Center** : classements et succès sont gérés par Apple via GameKit ;
  ...
  d'Apple, pas celle de Solea. → *Non collectée par Solea.*
...
Si une future version définit `SOLEA_COACH_PROXY_URL` (par ex. Solea Plus avec
