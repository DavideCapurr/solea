...
handled by Apple-managed services (WeatherKit, HealthKit, Game Center). Tanora
...
- **Location**: used in-app only to call Apple's WeatherKit for UV index and
  weather. Apple handles the request as a system service; Tanora does not log
  or transmit location anywhere else. → *Not collected by Tanora.*
- **Health & Fitness**: Tanora **writes** Time in Daylight to the user's
  HealthKit store when explicitly authorized. It does not read HealthKit
  data. Values stay in the user's Health store. → *Not collected by Tanora.*
- **Photos**: the photo diary saves user-selected images into the local
  SwiftData store (app sandbox). No upload to Tanora or third parties.
  → *Not collected by Tanora.*
- **Game Center**: leaderboards and achievements are managed by Apple via
  GameKit; Game Center data is covered by Apple's privacy policy, not Tanora's.
  → *Not collected by Tanora.*
...
If a future release sets `SOLEA_COACH_PROXY_URL` (e.g. Tanora Plus with
