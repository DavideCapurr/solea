...
final class NotificationService {
    ...
    /// Programma gli inviti ad aprire Tanora prima e all'inizio delle prossime
        ...
        for (index, window) in upcomingWindows.enumerated() {
            ...
            if approachingDate > now {
                try await scheduleGoldenHourReminder(
                    ...
                    body: String(localized: "La finestra migliore si avvicina. Apri Tanora per controllare UV, SPF e durata prima di esporti."),
            ...
            if window.start > now {
                try await scheduleGoldenHourReminder(
                    ...
                    body: String(localized: "Le condizioni sono tra le più favorevoli per il tuo fototipo. Entra in Tanora e avvia una sessione prudente."),
                    date: window.start,
                    calendar: calendar
                )
                scheduledCount += 1
